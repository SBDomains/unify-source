...
#if defined(NDEBUG)
# error "Unify cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Unify Signed Message:\n";
...
{
    // Unify: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 3000 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 181818 * COIN;
            }

    // Subsidy is cut in half every 3000 blocks
    nSubsidy >>= (nHeight / 3000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Unify: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Unify: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1497449710, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Residents safety warnings fell on deaf ears";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a0b0c1f1bcb431b6d8e886885d454fc252a404c6af42cb48410d319bf9445ba12bffaa625889204a243dd25a8cdd4e4f8db058f07332aa91187cc012182549cc") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x214037fe78c284b2ee501e0c68d629126d394a4fa04f23f88699af72ceedcccd"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0x47, 0xf9, 0xee }; // Unify: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnifyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnifyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnifyMiner(CWallet *pwallet)
{
    printf("UnifyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unify-miner");
    ...
    try { loop {
        ...
        printf("Running UnifyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnifyMiner, pwallet));
}
